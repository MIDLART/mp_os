...
template<
    typename tkey,
    typename tvalue>
typename b_tree<tkey, tvalue>::infix_iterator &b_tree<tkey, tvalue>::infix_iterator::operator++() // TODO
{
    if (_path.empty())
    {
        // undefined behavior
...
std::tuple<size_t, size_t, tkey const &, tvalue &> b_tree<tkey, tvalue>::infix_iterator::operator*() const
{
    ...
    return std::tuple<size_t, size_t, tkey const &, tvalue &>(
            _path.size() - 1, _path.top().second, std::cref(kvp.key), std::ref(kvp.value)); // TODO
}

#pragma endregion infix_iterator

#pragma region infix_const_iterator
...
{
    auto data = *_iterator;
    return std::tuple<size_t, size_t, tkey const &, tvalue const &>(
            std::get<0>(data), std::get<1>(data), std::cref(std::get<2>(data)), std::cref(std::get<3>(data))); // TODO
}

#pragma endregion infix_const_iterator

#pragma endregion iterators implementation

#pragma region insert, obtain, dispose
        ...
        typename associative_container<tkey, tvalue>::key_value_pair &&kvp)
{
    ...
    if (node == nullptr && path.size() == 1) // root (first element)
    {
        typename search_tree<tkey ,tvalue>::common_node *new_node;
        new_node = this->create_node(_t);
    ...
    // tree leaf indexes: 0, 1, 2, ... virtual_size - 1
    // subtrees indexes: -1, -2, -3, ... -virtual_size - 1
    ...
    while (true)
    {
        if (node->virtual_size < get_max_keys_count()) // there is a free place
        {
    ...
    tkey const &key)
{
    ...
    if ((*path.top().first)->subtrees[0] != nullptr)
    {
        ...
        search_tree<tkey, tvalue>::swap(
                std::move((*non_terminal_node_with_key_found_info.first)->keys_and_values[non_terminal_node_with_key_found_info.second]),
    ...
    while (true)
    {
        ...
        if (can_take_from_left_brother)
        {
            ...
            search_tree<tkey, tvalue>::swap(
                    std::move(parent->keys_and_values[position - 1]),
                    std::move(left_brother->keys_and_values[left_brother->virtual_size - 1]));

            allocator::construct(
                    target_node->keys_and_values + target_node->virtual_size,
                    std::move(left_brother->keys_and_values[left_brother->virtual_size - 1]));
            search_tree<tkey, tvalue>::swap(
                    std::move(left_brother->subtrees[left_brother->virtual_size]),
...
#pragma endregion insert, obtain, dispose
